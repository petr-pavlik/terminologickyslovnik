...
\begin{itemize}
  \item $\lambda E$ – latentní tok tepla (W·m⁻²),
  \item $R_n$ – tok čisté sálavé energie (W·m⁻²),
  \item $G$ – tok tepla do půdy (W·m⁻²),
  \item $\rho$ – hustota vzduchu (kg·m⁻³),
  \item $c_p$ – měrná tepelná kapacita vzduchu při stálém tlaku (J·kg⁻¹·K⁻¹),
  \item $(e_s - e_a)$ – rozdíl mezi nasyceným a skutečným tlakem vodní páry (Pa),
  \item $r_a$ – aerodynamický odpor (s·m⁻¹),
  \item $r_s$ – stomatární (povrchový) odpor (s·m⁻¹),
  \item $\Delta$ – směrnice křivky nasycení vodní párou (Pa·K⁻¹),
  \item $\gamma$ – psychrometrická konstanta (Pa·K⁻¹).
\end{itemize}
...
\begin{itemize}
  \item $c_p$ – měrná tepelná kapacita suchého vzduchu při stálém tlaku (J·kg⁻¹·K⁻¹),
  \item $P$ – atmosférický tlak (Pa),
  \item $\varepsilon$ – poměr molekulových hmotností vodní páry a suchého vzduchu (≈ 0,622),
  \item $\lambda$ – latentní výparné teplo vody (J·kg⁻¹).
\end{itemize}
...
**Potenciální evapotranspirace** (angl. *potential evapotranspiration*, zkr. *ETₚ*) je **množství vody, které by se mohlo odpařit a transpirovat z povrchu půdy a vegetace**, pokud by byla **k dispozici neomezená zásoba vody**.  
...
Hodnota *ETₚ* se obvykle určuje z empirických nebo fyzikálních rovnic, např. podle **Penman–Monteithovy** či **Thornthwaitovy** rovnice.
