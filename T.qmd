...
\begin{itemize}
  \item $A$ – rychlost asimilace CO$_2$,
  \item $E$ – rychlost transpirace,
  \item $\lambda$ – parametr vyjadřující poměr mezi hodnotou uhlíku a náklady na ztrátu vody.
\end{itemize}
...
\begin{itemize}
  \item $ET_m$ – měsíční potenciální evapotranspirace (mm),
  \item $T_m$ – průměrná měsíční teplota (°C),
  \item $L_m$ – průměrná délka dne (h),
  \item $N_m$ – počet dní v měsíci,
  \item $I$ – teplotní index, tj. součet $\left( \frac{T_m}{5} \right)^{1.514}$ za celý rok,
  \item $a$ – empirický exponent vypočítaný z hodnoty $I$ podle vztahu:
\end{itemize}
...
\begin{itemize}
  \item $ET_c$ – transpirační (celková) vláhová potřeba plodiny,
  \item $K_c$ – koeficient plodiny, vyjadřující poměr mezi výparem konkrétní plodiny a referenční evapotranspirací,
  \item $ET_0$ – referenční evapotranspirace stanovená pro standardní povrch.
\end{itemize}
...
\begin{itemize}
  \item $K_t$ – transpirační koeficient,
  \item $W$ – množství vody spotřebované rostlinou (obvykle v litrech nebo kilogramech),
  \item $B$ – množství vytvořené sušiny (v kilogramech).
\end{itemize}
...
\begin{itemize}
  \item $R_n$ – čistá radiační bilance,
  \item $G$ – tok tepla do půdy,
  \item $H$ – tok citelného tepla,
  \item $\lambda E$ – latentní tok tepla (evapotranspirace).
\end{itemize}
